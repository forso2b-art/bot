...
CREATOR_ID = 103161998  # ID создателя
ADMIN_IDS = {37607526}  # ID админов (без создателя)
...
waiting_promote_user_id = State() # Состояние для назначения админа
waiting_demote_user_id = State() # Состояние для снятия админа
waiting_ban_user_id = State() # Состояние для бана
...
# Дополнительные опции для создателя
...
"""Показать задачи пользователя"""
if user_id in BANNED_USERS:
await bot.send_message(chat_id, "❌ Вы заблокированы в боте.")
return
...
if callback.from_user.id not in ADMIN_IDS and callback.from_user.id != CREATOR_ID:
...
users = [uid for uid in db.users.keys() if uid not in BANNED_USERS] # Исключаем забаненных
success = 0
failed = 0
...
if user_id in BANNED_USERS:
await callback.answer("❌ Нельзя отправить сообщение забаненному пользователю!", show_alert=True)
return
...
if user_id in BANNED_USERS:
await message.answer("❌ Пользователь с таким ID заблокирован.")
return
...
await message.answer("❌ Нельзя отправить сообщение забаненному пользователю.")
return
...
await callback.answer("❌ Только создатель может назначать администраторов!", show_alert=True)
return
...
await state.set_state(AdminStates.waiting_promote_user_id)

@router.message(AdminStates.waiting_promote_user_id)
async def process_promote_user(message: Message, state: FSMContext):
"""Обработка ID пользователя для назначения админа"""
if message.from_user.id in BANNED_USERS:
await message.answer("❌ Вы заблокированы в боте.")
return
...
user_id = int(message.text)
if user_id == CREATOR_ID:
await message.answer("❌ Нельзя сделать создателя администратором!")
return
if user_id in ADMIN_IDS:
await message.answer("❌ Пользователь уже является администратором!")
return
if user_id in BANNED_USERS:
await message.answer("❌ Нельзя назначить администратором заблокированного пользователя!")
return
...
await message.answer(
f"✅ Пользователь ID: {user_id} (@{user.get('username', 'Без имени')}) назначен администратором!",
reply_markup=get_admin_keyboard(message.from_user.id)
)
await state.clear()
except ValueError:
...
"
"Введите ID пользователя, которого хотите снять с должности администратора:",
reply_markup=InlineKeyboardMarkup(
inline_keyboard=[[
InlineKeyboardButton(text="❌ Отмена", callback_data="admin_back")
]]
...
await state.set_state(AdminStates.waiting_demote_user_id)

@router.message(AdminStates.waiting_demote_user_id)
async def process_demote_user(message: Message, state: FSMContext):
"""Обработка ID пользователя для снятия админа"""
if message.from_user.id in BANNED_USERS:
await message.answer("❌ Вы заблокированы в боте.")
return
...
user_id = int(message.text)
if user_id == CREATOR_ID:
await message.answer("❌ Нельзя снять создателя с должности администратора!")
return
...
await message.answer(
f"✅ Пользователь ID: {user_id} (@{user.get('username', 'Без имени')}) снят с должности администратора!",
reply_markup=get_admin_keyboard(message.from_user.id)
)
await state.clear()
except ValueError:
...
if callback.from_user.id not in ADMIN_IDS and callback.from_user.id != CREATOR_ID:
await callback.answer("❌ У вас нет прав для бана пользователей!", show_alert=True)
return
...
user_id = int(message.text)
if user_id == CREATOR_ID:
await message.answer("❌ Нельзя заблокировать создателя!")
return
if user_id in BANNED_USERS:
...
try:
await bot.send_message(user_id, "❌ Вы были заблокированы в боте.")
except Exception:
pass # Игнорируем ошибку, если не можем отправить сообщение

await message.answer(
f"✅ Пользователь ID: {user_id} (@{user.get('username', 'Без имени')}) заблокирован!",
reply_markup=get_admin_keyboard(message.from_user.id)
)
await state.clear()
except ValueError:
...
if callback.from_user.id not in ADMIN_IDS and callback.from_user.id != CREATOR_ID:
await callback.answer("❌ У вас нет прав для разбана пользователей!", show_alert=True)
return
...
await state.set_state(AdminStates.waiting_ban_user_id) # Переиспользуем состояние
...
await message.answer(
f"✅ Пользователь ID: {user_id} (@{user.get('username', 'Без имени')}) разблокирован!",
reply_markup=get_admin_keyboard(message.from_user.id)
)
await state.clear()
except ValueError:
...
if callback.from_user.id != CREATOR_ID:
await callback.answer("❌ Только создатель может просматривать список забаненных!", show_alert=True)
return

if not BANNED_USERS:
text = "<b>🚫 Список забаненных пользователей (пусто)</b>"
else:
text = "<b>🚫 Список забаненных пользователей</b>
"
for i, user_id in enumerate(sorted(BANNED_USERS), 1):
user = db.users.get(user_id, {})
username = user.get('username', 'Неизвестно')
full_name = user.get('full_name', 'Неизвестно')
text += f"{i}. ID: <code>{user_id}</code> | @{username} | {full_name}
"
...
await show_user_tasks(callback.from_user.id, callback.message.chat.id, page=int(callback.data.split("_", 2)[2]))
